      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] font-bold text-[#16A34A] uppercase tracking-wider">GasFree · TRON</p>
          </div>
        </div>
    ...
    <div className="pt-6 space-y-5 animate-in fade-in duration-300">
      ...
      {loading && !principal ? (
        <div className="flex flex-col items-center py-16 gap-4">
          <div className="relative w-12 h-12"><div className="absolute inset-0 rounded-full border-4 border-slate-100" /><div className="absolute inset-0 rounded-full border-4 border-transparent border-t-[#4ADE80] animate-spin" /></div>
        ...
        <>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <button onClick={() => setCreateOpen(true)} className="rounded-2xl border-2 border-dashed border-slate-200 hover:border-[#4ADE80] hover:bg-green-50/40 p-4 flex flex-col items-center justify-center gap-2 text-slate-700 hover:text-[#16A34A] min-h-[140px] transition-colors font-bold">
            ...
            <div>
              ...
              <div className="space-y-2">
                {principal && sendFrom.id !== 'principal' && (
                  <button onClick={() => setSendDest('principal')} className={`w-full text-left rounded-xl border p-3 flex items-center gap-2 ${sendDest === 'principal' ? 'border-[#4ADE80] bg-green-50' : 'border-slate-200'}`}>
                    <Star size={15} className="text-[#4ADE80]" fill="#4ADE80" />
                ...
                <button onClick={() => setSendDest('other')} className={`w-full text-left rounded-xl border p-3 ${sendDest === 'other' ? 'border-[#4ADE80] bg-green-50' : 'border-slate-200'}`}>
                ...
                {sendDest === 'other' && (
                  <input value={sendAddr} onChange={(e) => setSendAddr(e.target.value)} placeholder="Dirección USDT (TRC-20)" className="w-full px-3 py-2.5 text-xs font-mono border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4ADE80]" />
                )}
              </div>
            </div>
            <div>
